...
- 自定义图片现在可选择“整窗背景”或兼容原有效果的“主页图片卡片”；未保存模式的旧安装继续使用主页卡片。
- 整窗模式会分析图片焦点、亮度、比例和安全留白，将侧栏、主页、任务页、标题栏与输入区协调为半透明可读层。
- Injector 会监测图片、主题、配色与显示模式变化并热更新已连接的 Codex 窗口，无需重新加载页面。
- 主题包位于 `windows/themes/<id>/theme.json`，背景图与配色独立，可让同一张自定义图片配合不同主题使用。
...
- 桌面现在只保留主启动快捷方式；安装、恢复、主题切换与图片工具快捷方式统一存放在仓库的 `windows/shortcuts/`，重新安装也不会再次铺满桌面。
...
- 渲染层回归测试现在覆盖主题变量、主题身份、亮暗外观元数据和清理流程；全部 8 套主题均需通过 payload 构建检查。
- 增加整窗/主页卡片双模式、本地模式持久化、自适应焦点变量和 Injector 热更新契约测试。
